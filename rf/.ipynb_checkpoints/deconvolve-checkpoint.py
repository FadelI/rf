...
#        winsrc[1] = lenrsp_sec - onset_sec
    ...
    if method == 'time':
        ...
        rsp_data = [tr.data for tr in rsp]
        rf_data, nit, lsfit, gau = deconv_iterative(rsp_data, src.data, sr, 
                                                  tshift=tshift, **kwargs)
        for i, tr in enumerate(rsp):
            tr.data = rf_data[i].real
            tr.stats['iterations'] = nit[i]
            tr.stats['lsfit'] = lsfit[i]
            tr.stats['gaussian'] = gau
...
def deconv_iterative(rsp, src, sampling_rate, tshift=10, gauss=0.5, itmax=1000,
